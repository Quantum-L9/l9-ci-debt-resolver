RESOLVER-P6 adds optional, fail-closed delegation to PR_Repair for cases the resolver cannot remediate directly. Delegation produces proposals only; Resolver retains authority over evidence, validation, mutation, push, attempts, and terminal states.

Save as build-phase-6.sh and run after P5.
...
required = [
“.l9/pr-repair-delegation-contract.yaml”,
“.l9/delegation-privacy-policy.yaml”,
“.l9/phase-6-acceptance-gates.yaml”,
“schemas/resolver/pr-repair-request.schema.json”,
“schemas/resolver/pr-repair-proposal.schema.json”,
“schemas/resolver/delegation-record.schema.json”,
“src/l9_debt_resolver/delegation/privacy.py”,
“src/l9_debt_resolver/delegation/identity.py”,
“src/l9_debt_resolver/delegation/builder.py”,
“src/l9_debt_resolver/delegation/proposal.py”,
“src/l9_debt_resolver/delegation/converter.py”,
“src/l9_debt_resolver/delegation/nonce_ledger.py”,
“src/l9_debt_resolver/delegation/ledger.py”,
“src/l9_debt_resolver/delegation/file_transport.py”,
“src/l9_debt_resolver/delegation/http_transport.py”,
“src/l9_debt_resolver/delegation/service.py”,
“src/l9_debt_resolver/runtime/delegation_service.py”,
“tests/delegation/test_identity.py”,
“tests/delegation/test_nonce_ledger.py”,
“tests/delegation/test_file_transport.py”,
“tests/delegation/test_proposal_signature.py”,
“tests/privacy/test_delegation_privacy.py”,
“tests/resilience/test_delegation_retry.py”,
“.github/workflows/phase-6-pr-repair-delegation.yml”,
]
...
if missing:
raise SystemExit(
f”RESOLVER-P6 required files missing: {missing}”
)
...
for term in prohibited:
    if term in content:
        raise SystemExit(
            f"prohibited RESOLVER-P6 behavior "
            f"{term!r} in {path}"
        )
...
if "subprocess" in content:
    raise SystemExit(
        f"delegation code cannot execute subprocesses: {path}"
    )
...
printf ‘\n’
printf ‘RESOLVER-P6 build complete.\n’
printf ‘\n’
printf ‘Implemented:\n’
printf ’  - typed PR_Repair delegation requests\n’
printf ’  - bounded privacy-safe context\n’
printf ’  - repository HMAC pseudonymization\n’
printf ’  - repository-path tokenization\n’
printf ’  - deterministic request identities\n’
printf ’  - idempotent file and HTTPS delivery\n’
printf ’  - bounded delivery retries\n’
printf ’  - signed proposal callbacks\n’
printf ’  - callback timestamp validation\n’
printf ’  - durable replay protection\n’
printf ’  - fingerprint and snapshot binding\n’
printf ’  - proposal scope and bounds validation\n’
printf ’  - protected-path enforcement after token resolution\n’
printf ’  - proposal conversion into Resolver remediation plans\n’
printf ’  - P3 validation and rollback re-entry\n’
printf ’  - retained Resolver push and rerun authority\n’
printf ’  - retained Resolver terminal-state authority\n’
printf ’  - no PR_Repair mutation, validation, push, or merge authority\n’
printf ‘\n’
...
printf ‘\n’
printf ‘Resolver roadmap complete through RESOLVER-P6.\n’

:::
A proposal accepted in P6 is not trusted as a finished repair. It becomes a normal Resolver remediation plan and must still pass P3 transaction and SDK validation, P4 push and rerun controls, and P5 privacy-safe outcome reporting.