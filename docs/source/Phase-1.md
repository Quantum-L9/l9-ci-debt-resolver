RESOLVER-P1 uses GitHub’s workflow-run and workflow-job APIs directly rather than depending on parsed gh run view output. GitHub documents dedicated endpoints for run metadata, jobs, and per-job logs; the CLI itself notes limitations when associating large numbers of job logs. 

The build implements the specification’s complete failed-log retrieval, truncation detection, artifact provenance, redaction, and bounded retries. repo-spec.yaml

Save as build-phase-1.sh in the P0 repository root.
...
15. Local structural validation
...
required = [
“.l9/log-acquisition-contract.yaml”,
“.l9/phase-1-acceptance-gates.yaml”,
“schemas/resolver/failed-run-reference.schema.json”,
“schemas/resolver/failed-job.schema.json”,
“schemas/resolver/log-provenance.schema.json”,
“schemas/resolver/acquisition-report.schema.json”,
“src/l9_debt_resolver/acquisition/config.py”,
“src/l9_debt_resolver/acquisition/models.py”,
“src/l9_debt_resolver/acquisition/redaction.py”,
“src/l9_debt_resolver/acquisition/completeness.py”,
“src/l9_debt_resolver/acquisition/retry.py”,
“src/l9_debt_resolver/acquisition/service.py”,
“src/l9_debt_resolver/acquisition/store.py”,
“src/l9_debt_resolver/providers/github/transport.py”,
“src/l9_debt_resolver/providers/github/parser.py”,
“src/l9_debt_resolver/providers/github/provider.py”,
“tests/acquisition/test_completeness.py”,
“tests/acquisition/test_service.py”,
“tests/providers/github/test_parser.py”,
“tests/privacy/test_log_redaction.py”,
“tests/resilience/test_retry.py”,
“.github/workflows/phase-1-log-acquisition.yml”,
]
...
if missing:
raise SystemExit(
f”RESOLVER-P1 required files missing: {missing}”
)
...
for term in prohibited:
    if term in content:
        raise SystemExit(
            f"prohibited RESOLVER-P1 behavior "
            f"{term!r} in {path}"
        )
...
if ‘“raw.log”’ in store:
raise SystemExit(
“raw CI log persistence is prohibited”
)

if ‘“redacted.log”’ not in store:
raise SystemExit(
“redacted log persistence is missing”
)
...
“redacted_log_persistence”: True,
“incomplete_evidence_behavior”: (
“insufficient_log_evidence”
),
“SDK_repository_correlation”: False,
“bounded_remediation”: False,
...
printf ‘\n’
printf ‘RESOLVER-P1 build complete.\n’
printf ‘\n’
printf ‘Implemented:\n’
printf ’  - GitHub Actions failed-run acquisition\n’
printf ’  - bounded failed-job pagination\n’
printf ’  - independent per-job log retrieval\n’
printf ’  - bounded retry and Retry-After support\n’
printf ’  - log byte limits\n’
printf ’  - truncation and completeness detection\n’
printf ’  - deterministic evidence identities\n’
printf ’  - cryptographic artifact provenance\n’
printf ’  - credential, email, and path redaction\n’
printf ’  - redacted-only durable artifact storage\n’
printf ’  - fail-closed incomplete evidence handling\n’
printf ’  - provider, privacy, resilience, and architecture tests\n’
printf ‘\n’
...
printf ‘\n’
printf ‘Next phase:\n’
printf ’  RESOLVER-P2 — SDK repository snapshots, stack-frame and test\n’
printf ’  correlation, applicable contracts, canonical finding correlation,\n’
printf ’  and CI-specific root-cause classification.\n’

:::
One implementation correction is worth noting before execution: the P0 `pyproject.toml` must include `pytest-asyncio`; this build inserts it and enables strict async-test mode.